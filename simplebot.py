# creates scrollable text window
...
# Generating response
def get_bot_response_fixed(user_input):
...
def send(event):
    ...
    bot_response = get_bot_response_gpt(user_input_lc) 
...
# Chat window - initially disabled (not user editable)
...
# Create user text entry box
userEntryBox = Text(baseWindow, bd=1, bg='white', width=38, font='Arial')

# Place the widgets where they should go.
